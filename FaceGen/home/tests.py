...
class GeneratedImageModelTests(TestCase):
    ...
    def setUp(self):
        GeneratedImage.objects.create(
            prompt="Test Prompt", negative_prompt="Test Negative Prompt", width=256, height=256, num_inference_steps=100, guidance_scale=0.1, seed=0
        )
        GeneratedImage.objects.create(
            prompt="Test Prompt 2", negative_prompt="Test Negative Prompt 2", width=512, height=512, num_inference_steps=200, guidance_scale=0.2, seed=1
        )
        GeneratedImage.objects.create(
            prompt="Test Prompt 3", negative_prompt="Test Negative Prompt 3", width=1024, height=1024, num_inference_steps=300, guidance_scale=0.3, seed=2
        )
...
class DreamboothModelTests(TestCase):
    ...
    def setUp(self):
        DreamboothModel.objects.create(identifier="sks", training_steps=100, batch_size=1)
        DreamboothModel.objects.create(identifier="dlk", training_steps=200, batch_size=2)
        DreamboothModel.objects.create(identifier="axk", training_steps=300, batch_size=3)

    def test_instance_creation(self):
        """Test if the instance is created correctly"""
        model = DreamboothModel.objects.get(id=1)
        self.assertEqual(model.identifier, "sks")
        self.assertEqual(model.training_steps, 100)
        self.assertEqual(model.batch_size, 1)
        self.assertEqual(model.path, f"{settings.MEDIA_ROOT}dreambooth/FaceGen_{model.token}", "Path is not correct")

    def test_token_uniqueness(self):
        """Test if the token is unique"""
        tokens = DreamboothModel.objects.values_list('token', flat=True)
        self.assertEqual(len(tokens), len(set(tokens)), "Tokens are not unique")
